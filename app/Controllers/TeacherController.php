...
class TeacherController extends BaseController
{
    ...
    /**
     * Generate a human-friendly unique course code.
     * Ensures the generated code does not already exist in the `courses` table.
     * @param string $title
     * @return string
     */
    private function generateCourseCode($title)
    {
        $base = preg_replace('/[^A-Za-z0-9]+/', '-', trim($title));
        $base = strtoupper(substr($base, 0, 30));
        $base = trim($base, '-');

        $attempt = 0;
        do {
            $suffix = '-' . strtoupper(substr(md5(uniqid((string) mt_rand(), true)), 0, 6));
            $code = $base ? ($base . $suffix) : ('COURSE' . $suffix);
            $exists = $this->courseModel->where('code', $code)->first();
            $attempt++;
        } while ($exists && $attempt < 5);

        // As a fallback, use timestamp-based code
        if ($exists) {
            $code = 'COURSE-' . time() . '-' . mt_rand(100, 999);
        }
    ...
    public function search()
    {
        ...
        try {
            $courses = $this->courseModel->like('title', $searchTerm, 'both')
            ...
            return $this->response->setJSON(['error' => 'An error occurred while searching'])->setStatusCode(500);
        }
    }
}
